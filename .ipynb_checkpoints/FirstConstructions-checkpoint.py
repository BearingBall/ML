...
def accuracy(X, y, weight, H, metric, kernel, x_test, y_test):
    ...
    for i in range(len(y_test)):
        pred = predict(X,y,weight, x_test[i], metric, kernel, H)
        if pred == y_test[i]:
            TPN+=1
...
def DrawDatasetW(X, y, weight):
    plt.rcParams['figure.figsize'] = [15, 15]
    
    non_zero_ids = [i for i in np.arange(weight.shape[0]) if weight[i] > 0]
    reduced_x = X[non_zero_ids]
    reduced_y = y[non_zero_ids]
    reduced_dataframe = pd.DataFrame(reduced_x)
    scatter_matrix(reduced_dataframe, c=reduced_y,marker='o', s=40,alpha=.8)
    
    zero_ids = [i for i in np.arange(weight.shape[0]) if weight[i] == 0]
    unreduced_x = X[zero_ids]
    unreduced_y = y[zero_ids]
    unreduced_dataframe = pd.DataFrame(unreduced_x)
    scatter_matrix(unreduced_dataframe, c=unreduced_y, marker='x', s=40,alpha=.8)
